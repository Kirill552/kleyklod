...
GTIN_MISMATCH_TEXT = """
⚠️ <b>Баркоды не совпадают с кодами маркировки</b>

Баркоды в Excel отличаются от GTIN в кодах ЧЗ.
Это бывает у СНГ-селлеров с внутренними баркодами WB.

<b>Решение:</b>
Используйте веб-версию для ручного сопоставления товаров:
🌐 kleykod.ru/app/generate

💡 Если у вас <b>один товар</b> — попробуйте ещё раз,
система автоматически сопоставит его с кодами.
"""
...
async def process_generation(
    ...
    if not result.success:
        ...
        elif result.status_code == 422 and result.data and "gtin_matching_error" in result.data:
            # Ошибка матчинга GTIN — баркоды не совпадают с кодами ЧЗ
            await processing_msg.edit_text(
                GTIN_MISMATCH_TEXT,
                reply_markup=get_main_menu_kb(),
                parse_mode="HTML",
