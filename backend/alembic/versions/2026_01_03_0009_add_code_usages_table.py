...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('code_hash', sa.String(64), nullable=False, comment='SHA-256 хеш кода маркировки'),
        sa.Column('generation_id', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('used_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False, comment='Когда код был использован'),
